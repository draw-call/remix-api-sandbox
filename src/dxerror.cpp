...
HRESULT DXTrace(const WCHAR* strFile, WORD dwLine, HRESULT hr, const WCHAR* strMsg, bool bPopMsgBox)
{
  ...
  WCHAR strBuffer[DXTRACE_BUFFER_SIZE];

  swprintf_s(strBufferLine, 128, L"%lu", dwLine);
  if (strFile) {
    swprintf_s(strBuffer, DXTRACE_BUFFER_SIZE, L"%ls(%ls): ", strFile, strBufferLine);
  ...
  swprintf_s(strBuffer, DXTRACE_BUFFER_SIZE, L"hr=%ls", strBufferError);
  OutputDebugStringW(strBuffer);
  ...
  if (bPopMsgBox) {
    ...
    swprintf_s(strBuffer, DXTRACE_BUFFER_SIZE, L"File: %ls\nLine: %ls\nError Code: %ls\n%lsDo you want to debug the application?",
...
//--------------------------------------------------------------------------------------
HRESULT DXUTTrace(const CHAR* strFile, DWORD dwLine, HRESULT hr,
...
const WCHAR* WINAPI DXGetErrorStringW(HRESULT hr)
{
  switch (hr) {
    ...
    // -------------------------------------------------------------
    // Commmented out codes are actually alises for other codes or otherwise not useful currently
    CHK_ERRA(S_OK)
    CHK_ERRA(S_FALSE)
