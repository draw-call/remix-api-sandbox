...
// Copyright 2024 (c) Dayton Pidhriney (watbulb)
...
//--------------------------------------------------------------------------------------
HRESULT CMeshLoader::Create(IDirect3DDevice9* pd3dDevice, std::filesystem::path &objfilename)
{
  HRESULT hr;
  ...
  V_RETURN(LoadGeometryFromOBJ(objfilename));
...
//--------------------------------------------------------------------------------------
HRESULT CMeshLoader::LoadGeometryFromOBJ(std::filesystem::path &objfilepath)
{
  ...
  if (pMaterial == NULL)
    return DXTRACE_ERR(L"OOM", E_FAIL);
  ...
  wifstream InFile(objfilepath);
  if (!InFile) {
    DXTRACE_ERR(L"wifstream::open", E_FAIL);
    return E_FAIL;
  }
  ...
  if (strMaterialFilename[0]) {
    std::filesystem::path mtlfilepath(strMaterialFilename);
    V_RETURN(LoadMaterialsFromMTL(mtlfilepath));
  }
...
//--------------------------------------------------------------------------------------
HRESULT CMeshLoader::LoadMaterialsFromMTL(std::filesystem::path &mtlfilepath)
{
  HRESULT hr = 0;

  // File input
  WCHAR strCommand[256] = { 0 };
  wifstream InFile(mtlfilepath);
  if (!InFile)
