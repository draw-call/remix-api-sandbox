...
#include "skel/cdc.hpp"
#include "skel/d3d9ex.hpp"

namespace skel {

#define MIN_DEPTH 0.98f
#define MAX_DEPTH 9.8e7f
...
HRESULT d3d9ex::D3D9Device::SetVertexShaderConstantF(UINT StartRegister, CONST float* pConstantData, UINT Vector4fCount)
{
...
#ifdef NEXTGEN_SHADER_MODE // nextgen shaders are too advanced
    memcpy(world.m,      (float **)&s_PCStateManager->m_worldMatrix,      sizeof(world.m));
    memcpy(view.m,       (float **)&s_PCStateManager->m_viewMatrix,       sizeof(view.m));
    memcpy(projection.m, (float **)&s_PCStateManager->m_projectionMatrix, sizeof(projection.m));
#endif

    if (cdc::GameState->GetDeviceManager()->m_pCurrentRenderTarget) {
      // cdc::s_PCDeviceManager->m_pCurrentRenderTarget->m_viewport.zFar  = 1.0000000f;
      // cdc::s_PCDeviceManager->m_pCurrentRenderTarget->m_viewport.zNear = 0.0000001f;
      cdc::GameState->GetDeviceManager()->m_pCurrentRenderTarget->m_viewport.skySphereMode = true;
      cdc::GameState->GetDeviceManager()->m_pCurrentRenderTarget->m_useMultiSample = true;
    }

    //// if not XYZ, just return
    if (StartRegister != 0) {
    ...
    //// Get keystate to see if we need to move
    UCHAR VkKey;
    for (VkKey = 0; VkKey < UCHAR_MAX; VkKey++) {
      if ((GetAsyncKeyState(VkKey) & (1 << 15)) != 0) [[unlikely]] {
        cdc::GameState->HandleTranslateKey(VkKey);
      }
    }
...
HRESULT __stdcall d3d9ex::_d3d9::CreateDevice(UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags, D3DPRESENT_PARAMETERS* pPresentationParameters, IDirect3DDevice9** ppReturnedDeviceInterface)
{
    ...
    D3DXMatrixIdentity(&cdc::GameState->world);
    D3DXMatrixLookAtRH(&cdc::GameState->view, &cdc::GameState->camVec, &cdc::GameState->posVec, &cdc::GameState->axiVec);
    ...
    HRESULT r;
    if ((r = m_pIDirect3D9->CreateDevice(Adapter, DeviceType, hFocusWindow, BehaviorFlags, pPresentationParameters, ppReturnedDeviceInterface)) != S_OK) {
      return r;
    }
