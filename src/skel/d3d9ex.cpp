...
HRESULT d3d9ex::D3D9Device::BeginScene()
{
  ...
  // Begin new D3D Scene and update per-frame metadata
  result = m_pIDirect3DDevice9->BeginScene();

  // TODO: Can't do this if TRLAU is active for now, need to just get the address
  static auto DM = cdc::GameState->GetDeviceManager();
  if (DM && kanan::isGoodWritePtr(reinterpret_cast<uintptr_t>(DM->m_pCurrentRenderTarget), 4)) {
    // DM->m_pCurrentRenderTarget->m_viewport.zFar  = 1.0000000f;
    // DM->m_pCurrentRenderTarget->m_viewport.zNear = 0.0000001f;
    DM->m_pCurrentRenderTarget->m_viewport.skySphereMode = true;
    DM->m_pCurrentRenderTarget->m_useMultiSample = false;
  }

  //// Shuffle through keystate accel to see if we need to move multiple vectors
  UCHAR VkKey;
  for (VkKey = 0; VkKey < UCHAR_MAX; VkKey++) {
    if ((GetAsyncKeyState(VkKey) & (1 << 15)) != 0) [[unlikely]] {
      if (cdc::GameState->HandleTranslateKey(VkKey)) {
        m_pIDirect3DDevice9->SetTransform(D3DTS_VIEW, &cdc::GameState->view);
      }
      cdc::GameState->HandleFOVKey(VkKey);
    }
  }

  // Set FOV every frame, as it will slowly be reset back to the inital FOV by the engine if we don't
  cdc::GameState->CameraUpdateProjectionDistance();

  // Setup the transforms, etc once we've loaded
  // TODO: Detect level swap event
  if (!cdc::GameState->renderReady &&
       cdc::GameState->frameNumber++ == cdc::GameState->frameStartNumber) [[unlikely]] {

    // D3DXMATRIX  trans;
    // D3DXVECTOR3 campos = (D3DXVECTOR3 &)(cdc::GameState->GetCamera()->core.position);
    cdc::GameState->camVec = D3DXVECTOR3 {0.0f, 0.0f,   2.26f};  // z=~2.26 without world transform
    cdc::GameState->posVec = D3DXVECTOR3 {0.0f, 1.0f ,  0.0f};  // position where to look at (sometimes z=45.0)
    cdc::GameState->axiVec = D3DXVECTOR3 {0.0f, 0.0f,   1.0f};  // positive z-axis is up
    // D3DXMatrixTranslation(&trans, campos.x, campos.y, campos.z);
    D3DXMatrixLookAtRH(
      &cdc::GameState->view,
      &cdc::GameState->camVec,
      &cdc::GameState->posVec,
      &cdc::GameState->axiVec
    );
    m_pIDirect3DDevice9->SetTransform(D3DTS_WORLD, (const D3DXMATRIX *)&cdc::GameState->ident); // inverse of cwTransform2f is ident 
    m_pIDirect3DDevice9->SetTransform(D3DTS_VIEW,  (const D3DXMATRIX *)&cdc::GameState->view);  // view could also just be cwTransform itself, but we reverse engineer it here instead
    m_pIDirect3DDevice9->SetTransform(D3DTS_PROJECTION, &cdc::GameState->projection);

    // Matrix debugging light
    {
      ZeroMemory(&cdc::GameState->debugLight, sizeof(D3DLIGHT9));
      cdc::GameState->debugLight.Type         = D3DLIGHT_SPOT;
      cdc::GameState->debugLight.Diffuse      = D3DXCOLOR(0.5f, 0.5f, 0.5f, 1.0f);
      cdc::GameState->debugLight.Direction    = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
      cdc::GameState->debugLight.Position     = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
      cdc::GameState->debugLight.Range        = 1.0f;
      cdc::GameState->debugLight.Attenuation0 = 0.0f;
      cdc::GameState->debugLight.Attenuation1 = 0.125f;
      cdc::GameState->debugLight.Attenuation2 = 0.0f;
      cdc::GameState->debugLight.Phi          = D3DXToRadian(40.0f);
      cdc::GameState->debugLight.Theta        = D3DXToRadian(20.0f);
      cdc::GameState->debugLight.Falloff      = 1.0f;
    }
    cdc::GameState->renderReady = true;
  } // renderReady && frameNumber == frameStartNumber

  // Set this-frame metadata
  cdc::GameState->inScene      = true;
  cdc::GameState->sceneSetPosN = 0;

  // Activate custom lights every frame
  m_pIDirect3DDevice9->SetLight(0, &cdc::GameState->debugLight);
...
{
  // Generate our ImGui frame as the final set of drawcalls before EndScene and Present
  // Because BeginScene shares a different thread than EndScene sometimes, we must use EndScene to submit the DrawData
  // as it shares the MainThread when called.
  if (cdc::GameState->isDiagWindowFocused && cdc::GameState->renderReady && cdc::GameState->inScene) {
    menu::RenderDiagView(cdc::GameState);
  }

  cdc::GameState->inScene = false;
...
{
  HRESULT vp = m_pIDirect3DDevice9->SetViewport(pViewport);
  return vp;
}
...
{
  // Always disable culling -- no point wasting a branch to check the State.
  if (State == D3DRS_CULLMODE) {
    return m_pIDirect3DDevice9->SetRenderState(D3DRS_CULLMODE, D3DCULL_NONE);
  }

  // Some items you may want to disable if transitioning to FF pipeline.
  // m_pIDirect3DDevice9->SetRenderState(D3DRS_SCISSORTESTENABLE, FALSE);
  // m_pIDirect3DDevice9->SetRenderState(D3DRS_STENCILENABLE,     FALSE);
  // m_pIDirect3DDevice9->SetRenderState(D3DRS_FOGENABLE,         FALSE);
...
HRESULT d3d9ex::D3D9Device::SetTextureStageState(DWORD Stage, D3DTEXTURESTAGESTATETYPE Type, DWORD Value)
{
  ...
  // To disable vertex color and alpha blending
  //m_pIDirect3DDevice9->SetTextureStageState(Stage, Type, Value);
  //m_pIDirect3DDevice9->SetTextureStageState(0, D3DTSS_COLORARG0, D3DTOP_DISABLE);
  //m_pIDirect3DDevice9->SetTextureStageState(0, D3DTSS_COLORARG1, D3DTOP_DISABLE);
  //m_pIDirect3DDevice9->SetTextureStageState(0, D3DTSS_COLORARG2, D3DTOP_DISABLE);
  //m_pIDirect3DDevice9->SetTextureStageState(0, D3DTSS_ALPHAARG0, D3DTOP_DISABLE);
  //m_pIDirect3DDevice9->SetTextureStageState(0, D3DTSS_ALPHAARG1, D3DTOP_DISABLE);
  //return m_pIDirect3DDevice9->SetTextureStageState(0, D3DTSS_ALPHAARG2, D3DTOP_DISABLE);
}
...
{
  return m_pIDirect3DDevice9->SetFVF(FVF | D3DFVF_DIFFUSE | D3DFVF_TEX1);
}
...
// c12 - TEX2 
// NOTE:
//   If you want to override these shader constants, you may update the StartRegister if statement below and modify pConstantData buffer
//   based on the index layout displayed above. This can be useful if you need to reverse engineer some specifics
//   of a specific draw call when used with a shader.
HRESULT d3d9ex::D3D9Device::SetVertexShaderConstantF(UINT StartRegister, CONST float* pConstantData, UINT Vector4fCount)
{
  if (cdc::GameState->inScene && StartRegister == 0) {
    cdc::GameState->lastConstData[cdc::GameState->sceneSetPosN++] = {true, pConstantData};
  }
...
HRESULT __stdcall d3d9ex::_d3d9::CreateDevice(UINT Adapter, D3DDEVTYPE DeviceType, HWND  hFocusWindow, DWORD BehaviorFlags, D3DPRESENT_PARAMETERS* pPresentationParameters, IDirect3DDevice9** ppReturnedDeviceInterface)
{
  ...
  // Everytime the resolution is reset in-game, which subsequently calls CreateDevice again,
  // we need to get the new rect size for the focused window size for the projection.
  RECT rc;
  GetWindowRect(hFocusWindow, &rc);
  cdc::GameState->CreateProjectionTransform(rc.right, rc.bottom);

  // Setup ImGui on the main focused window just before device creation.
  // This ensures the context is bound to the game main-thread which manages the device creation and draw call submission,
  // otherwise we would need to share a context and allocator between threads, when we only want to
  // share the context itself, pedantically. We cannot create out ImGui context in the injected DLL thread, the heap is not shared.
  cdc::GameState->SetupImGuiW32(std::bind(cdc::GameStateWndProcCallback, cdc::GameState, _1, _2, _3, _4), hFocusWindow);

  // If we already had an active render session and are being reset, inform ImGui.
  // Later the game will call device->Reset(). If no device present, is no-op.
  if (cdc::GameState->renderReady) {
    cdc::GameState->ResetImGuiDX9();
    cdc::GameState->renderReady = false;
    cdc::GameState->inScene     = false;
    cdc::GameState->frameNumber = 0;
  }

  // Create Device and set interface pointer
  if ((hRes = m_pIDirect3D9->CreateDevice(Adapter, DeviceType, hFocusWindow, BehaviorFlags, pPresentationParameters, ppReturnedDeviceInterface)) != S_OK) {
    return hRes;
  }
  *ppReturnedDeviceInterface = new d3d9ex::D3D9Device(*ppReturnedDeviceInterface);

  // Pass the interface device pointer to ImGui
  cdc::GameState->SetupImGuiDX9(*ppReturnedDeviceInterface);

  // This doesn't seem to work ... anyways ...
  cdc::o_TRANS_SetTransformMatrixf = reinterpret_cast<cdc::t_TRANS_SetTransformMatrixf>(0x402B10);
